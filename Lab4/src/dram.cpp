...
DRAM *dram_new()
{
    ...
    RowBuffer *row_buffers = new RowBuffer[NUM_BANKS];
    DRAM *dram = new DRAM;
    dram->row_buffers = row_buffers;
    return dram;
}
...
uint64_t dram_access(DRAM *dram, uint64_t line_addr, bool is_dram_write)
{
    ...
    dram->stat_read_delay += (!is_dram_write) ? delay : 0;
    dram->stat_write_delay += (is_dram_write) ? delay : 0;
    return delay;
}
                               ...
                               bool is_dram_write)
{
    ...
    if(DRAM_PAGE_POLICY == CLOSE_PAGE) {
    ...
    } else if(DRAM_PAGE_POLICY == OPEN_PAGE) {
        uint64_t row_id = line_addr / (ROW_BUFFER_SIZE / CACHE_LINESIZE);
        uint64_t bank = row_id % NUM_BANKS;
        RowBuffer *row_buffer = &(dram->row_buffers[bank]);
        if(row_buffer->valid && dram->row_buffers[bank].row_id == row_id) {
        ...
        dram->row_buffers[bank].row_id = row_id;
        return delay + DELAY_ACT + DELAY_CAS + DELAY_BUS;
    }
    return 1;
}
