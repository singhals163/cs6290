                             ...
                             AccessType type, unsigned int core_id)
{
    ...
    if (needs_dcache_access)
    {
        ...
        {
            #ifdef DEBUG
            printf("Cache installing cycle: %ld\n", current_cycle);
            #endif
                              ...
                              AccessType type, unsigned int core_id)
{
    ...
    if (type == ACCESS_TYPE_IFETCH)
    {
        ...
        CacheResult outcome_icache = cache_access(sys->icache, line_addr, false,
                                           core_id);
        delay += ICACHE_HIT_LATENCY;
        if (outcome_icache == MISS)
        {
            delay += memsys_l2_access(sys, line_addr, false, core_id);
            cache_install(sys->icache, line_addr, false, core_id);
            CacheLine *last_evicted_line = &(sys->icache->last_evicted_line);
            if(last_evicted_line->valid && last_evicted_line->dirty) {
                memsys_l2_access(sys, last_evicted_line->line_addr, true, last_evicted_line->core_id);
            }
            last_evicted_line->valid = false;
            last_evicted_line->dirty = false;
        }
        #ifdef DEBUG
        printf("ICACHE Delay: %ld\n", delay);
        #endif
        return delay;
    }
    ...
    if(needs_dcache_access) {
        CacheResult outcome_dcache = cache_access(sys->dcache, line_addr, is_write, core_id);
        delay += DCACHE_HIT_LATENCY;
        if(outcome_dcache == MISS) {
            delay += memsys_l2_access(sys, line_addr, false, core_id);
            cache_install(sys->dcache, line_addr, is_write, core_id);
            CacheLine *last_evicted_line = &(sys->dcache->last_evicted_line);
            if(last_evicted_line->valid && last_evicted_line->dirty) {
                memsys_l2_access(sys, last_evicted_line->line_addr, true, last_evicted_line->core_id);
            }
            last_evicted_line->valid = false;
                          ...
                          bool is_writeback, unsigned int core_id)
{
    ...
    if(res == MISS) {
        ...
        CacheLine *last_evicted_line = &(sys->l2cache->last_evicted_line);
        if(last_evicted_line->valid && last_evicted_line->dirty) {
            dram_access(sys->dram, last_evicted_line->line_addr, true);
        }
        last_evicted_line->valid = false;
        last_evicted_line->dirty = false;
    }
                               ...
                               AccessType type, unsigned int core_id)
{
    ...
    p_line_addr =  memsys_convert_vpn_to_pfn(sys, v_line_addr / (PAGE_SIZE / CACHE_LINESIZE), core_id) * (PAGE_SIZE / CACHE_LINESIZE) + (v_line_addr % (PAGE_SIZE / CACHE_LINESIZE)); // Replace this with a correct implementation.

    if (type == ACCESS_TYPE_IFETCH)
    {
        // TODO: Simulate the instruction fetch and update delay accordingly.
        CacheResult outcome_icache = cache_access(sys->icache_coreid[core_id], p_line_addr, false,
                                           core_id);
        delay += ICACHE_HIT_LATENCY;
        if (outcome_icache == MISS)
        {
            ...
            CacheLine *last_evicted_line = &(sys->icache_coreid[core_id]->last_evicted_line);
            if(last_evicted_line->valid && last_evicted_line->dirty) {
                memsys_l2_access(sys, last_evicted_line->line_addr, true, last_evicted_line->core_id);
            }
            last_evicted_line->valid = false;
    ...
    if(needs_dcache_access) {
        CacheResult outcome_dcache = cache_access(sys->dcache_coreid[core_id], p_line_addr, is_write, core_id);
        delay += DCACHE_HIT_LATENCY;
        if(outcome_dcache == MISS) {
            delay += memsys_l2_access(sys, p_line_addr, false, core_id);
            cache_install(sys->dcache_coreid[core_id], p_line_addr, is_write, core_id);
            CacheLine *last_evicted_line = &(sys->dcache_coreid[core_id]->last_evicted_line);
            if(last_evicted_line->valid && last_evicted_line->dirty) {
                memsys_l2_access(sys, last_evicted_line->line_addr, true, last_evicted_line->core_id);
            }
            last_evicted_line->valid = false;
